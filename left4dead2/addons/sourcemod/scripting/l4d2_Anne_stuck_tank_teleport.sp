			...
			if (GetSurvivorCountAlive() >= g_hCvarRusherMinPlayers.IntValue) {
				...
				if (tank != 0) {
					...
					if (distance > g_hCvarRusherDist.FloatValue) {
						...
						if (g_iRushTimes[i] >= g_hCvarRusherCheckTimes.IntValue && L4D2Direct_GetFlowDistance(tank)!= 0.0 && L4D2Direct_GetFlowDistance(i)!=0.0&& L4D2Direct_GetFlowDistance(tank)<L4D2Direct_GetFlowDistance(i) && !L4D_IsMissionFinalMap() && !SAFEDETECT_IsEntityInEndSaferoom(i)) {
